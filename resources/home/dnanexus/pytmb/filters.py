...
def isPolym(v, infos, flags, val):
    ...
    v : cyvcf2.Variant
        The variant record (not used directly, kept for API consistency).
    infos : dict or list of dict
        Either a flat INFO dict (``dict(variant.INFO)``) when flags are string
        field names, or the parsed CSQ list-of-dicts from ``infoTag2dl`` when
        flags are integer CSQ field indices.
    flags : list of str or list of int
        INFO field names (str) or CSQ positional indices (int) that carry
    ...
    if isinstance(infos, list):
        # CSQ-style: list of per-transcript annotation dicts keyed by index
        for annot in infos:
            for idx in flags:
                af = annot.get(idx)
                if af is not None and af != "." and af != "NA":
                    try:
                        if float(af) >= float(val):
                            return True
                    except ValueError:
                        pass
        return False
